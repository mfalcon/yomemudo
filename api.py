...
def best():
    ...
    for origen, coord in origenes.items():
        tiempo_total = 0
        for destino in data:
            import pdb; pdb.set_trace()
            tipo_transporte = destino['tipo_transporte']
            tiempo_viaje = get_trips(coord['x'], coord['y'], destino['destino'][0], destino['destino'][1], destino['tipo_transporte'])
            tiempo_total += tiempo_viaje * int(destino['dias_semana']) * (2 if destino['trayecto'] == 'idavta' else 1)
            
        resultados.append({'origen': origen, 'tiempo_total': tiempo_total})
        print "tiempo total: {0}, origen: {1}".format(tiempo_total, origen)
...
def best_improv():
    ...
    for origen_sel in sorted(origenes_km, key=lambda x: x['recorrido_total'])[:total_origenes]:
        #resultados.append(get_trip_time(origen_sel, data))
        tiempo_total = 0
        #origen_x, origen_y = geocoding.barrios_coords[origen_sel['origen']][0], geocoding.barrios_coords[origen_sel['origen']][1]
        for destino in data:
            tipo_transporte = destino['tipo_transporte']
            #Para la funcion get_trips necesito las coordenadas
...
@app.route('/walkscore', methods = ['GET'])
@cross_origin() # Send Access-Control-Allow-Headers
def get_walkscore():
    data = walkscore.the_walkscore('data/puntos_cuadricula_caba.csv', 4,3, ',',colegios_publicos_caba, 24, 23, ';')
